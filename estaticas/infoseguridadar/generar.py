...
.media.sinfoto{--gx:18%;--gy:0%;--ang:160deg;background:
  radial-gradient(120% 100% at var(--gx) var(--gy), rgba(43,224,107,.30) 0%, rgba(43,224,107,0) 58%),
  radial-gradient(90% 80% at 100% 100%, rgba(43,224,107,.12) 0%, rgba(0,0,0,0) 60%),
  linear-gradient(var(--ang),#0D1512 0%,#050706 55%,#0A0F0D 100%);
...
.media.sinfoto::before{content:'';position:absolute;inset:0;opacity:.16;
  ...
  -webkit-mask-image:radial-gradient(120% 90% at var(--gx) 10%,#000 0%,transparent 70%)}
...
(function(){
  ...
  var agua=document.querySelector('.marca-agua');
  if(agua){
    var caja=agua.parentElement, a=parseFloat(getComputedStyle(agua).fontSize);
    var util=caja.clientWidth-88;
    while(agua.scrollWidth>util&&a>34){a-=2;agua.style.fontSize=a+'px';}
  }
...
# Focos de luz de la portada de respaldo: evita que dos piezas seguidas se vean iguales.
LUCES = [("18%", "0%", "160deg"), ("82%", "6%", "200deg"), ("50%", "0%", "140deg"),
         ("10%", "40%", "185deg"), ("95%", "50%", "150deg"), ("35%", "100%", "210deg")]


def armar_html(nota: dict, indice: int = 0) -> str:
    foto = nota.get("foto")
    ...
    if ruta and ruta.exists():
        ...
        gx, gy, ang = LUCES[indice % len(LUCES)]
        media = (
            f'<div class="media sinfoto" style="--gx:{gx};--gy:{gy};--ang:{ang}">'
            f'<span class="marca-agua">{agua}</span></div>'
        )
...
def main() -> int:
    ...
    for i, nota in enumerate(notas, 1):
        ...
        destino.write_text(armar_html(nota, i - 1), encoding="utf-8")
